...
module "ec2_publica" {
  ...
  user_data = file("./modules/ec2/automatizacao_front.sh")

  # user_data = templatefile("./modules/ec2/automatizacao_front.sh", {
  #   public_ip = module.elastic_ip_ec2.public_ip
  # })
...
module "sg_privado" {
  ...
  },
  {
    ...
    protocol = "tcp"
    cidr_blocks = ["10.0.0.0/25"]
    description = "Allow traffic from Nginx to backend on port 8080"
    ipv6_cidr_blocks = []
    prefix_list_ids = []
    security_groups = []
    self = false
...
module "route53" {
  ...
  vpc_id             = module.vpc.id
  private_ip_address = module.ec2_privada.private_ip
}